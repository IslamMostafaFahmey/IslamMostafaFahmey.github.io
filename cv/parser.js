...
function buildContent(cv) {
  ...
  for (let i in cv.organization1) {
    ...
    rv.push(
      `<div class="key${half ? " half" : ""}"><div class="title">${titleCase(
        header1
      )}</div>`
    );
    if (["work experience", "skills"].includes(header1)) {
      for (let j in cv.organization2) {
        ...
        for (let k in section2) {
          ...
          rvx.push(
            (section3.date
              ? `<div class="job date">${section3.date[0]} -<br>${section3.date[1]}</div>`
              : "") +
              `<div class="job desc ${
                header1 == "skills" ? "skill" : ""
              }"><div class="job title">${section3.title}</div>` +
    ...
    if (["courses", "awards and honors"].includes(header1)) {
      ...
      for (let k in section1) {
        ...
        rvx.push(
          (section3.date
            ? `<div class="job date">${section3.date}</div>`
            : "") +
            `<div class="job desc"><div class="job title">${section3.title}</div>` +
    ...
    if (
      ...
      for (let k in section1) {
        ...
        section3 = section1[k];
        rvx.push(
          "<li><b>" + section3.title + "</b> - " + section3.level + "</li>"
        );
      }
      rv.push(`<ul class="languages">${rvx.join("")}</ul>`);
    }
    if (["soft skills", "interests", "graduation project"].includes(header1)) {
  ...
  return `<div class="container"><div class="key small-font">${
    cv.pitch
  }</div><hr>${rv.join("").replace("<hr><hr>", "<hr>")}</div>`;
}
